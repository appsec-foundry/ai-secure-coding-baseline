...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check("the migration prompt names the file, the benefit, and stays short",
          ...
          and any(prompt.startswith(
              "Switch to a managed copy of aiscb-0.1.11, so updates reach it?")
...
check("the remote bootstrap pins and hashes one coherent bundle",
      "bundle_ref=\"aiscb-bundle-0.1.10-3\"" in setup_content
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    environment["aiscb_test_ref"] = "aiscb-bundle-0.1.10-3"
