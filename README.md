...
The goal is secure code by default: the agent applies these practices on its own, also when the prompt does not mention security at all.

Anthropic's own security team works this way. It encodes secure-coding guidelines in `CLAUDE.md` files and org-wide skills "so the code follows these best practices the minute it's generated," and closes the loop by updating those files whenever an agent finds a new bug class ([Clinton, 2026](https://claude.com/blog/how-anthropic-secures-its-ai-native-software-development-lifecycle)). The instruction file is where security enters an AI-native codebase. Everything else in that article—deterministic and agentic reviews in CI, humans at the highest-leverage points—is the layer that has to sit behind it.

Three research findings explain what such a file has to look like, and each is visible in how this baseline is written.

**Instructions the agent can follow, not information it should know.** [Gloaguen et al. (2026)](https://arxiv.org/abs/2602.11988) tested repository context files on real tasks: the project descriptions most templates start with had no effect, while explicit instructions in the same files were followed. Precision matters too—[Kharma et al. (2026)](https://arxiv.org/abs/2605.24298) found that asking a model to be security-aware did not reduce the number of vulnerabilities, it only changed which ones appeared. So every line of the baseline names a mechanism—authorize on the server, use parameterized queries, take the first admin credential from external configuration—instead of a goal the agent has to interpret.

**Name the shortcut, not just the principle.** A deadline or a failing test is the moment security gets dropped. In the AgentPressureBench study above, pressured agents took shortcuts in every task; rules that forbade exactly that behavior cut it from 100% to 8.3%. The baseline therefore forbids the act itself: never weaken a control so that code works or a test passes, and a control that can be switched off is already weakened, whether the switch is a flag, an environment variable, or a bypass called temporary. The four highest-risk rules are marked non-negotiable for the same reason. The remaining 8.3% is why this is a guardrail, not a control.

**Rules read once at the start decay.** [Gamage (2026)](https://arxiv.org/abs/2604.20911) found that prohibitions weaken the longer a session runs, while requirements in positive form stay stable; [Shukla et al. (2025)](https://arxiv.org/abs/2506.11022) saw critical vulnerabilities increase after only five rounds of a model improving its own code. Compliance drops exactly where agentic work spends most of its time. The baseline answers with a closing step: the agent reviews its own diff and reports what it found and fixed, the affected controls, the test results, and the open gaps. A lasting fix has to come from the model level, through instruction-hierarchy training ([Wallace et al., 2024](https://arxiv.org/abs/2404.13208)), not from a file like this one.
