...
**And it has to survive the session.** Pressure is the moment security gets dropped first: in the AgentPressureBench study above, pressured agents cut corners in every task, and rules forbidding exactly that behavior brought it down to 8.3%. So the baseline forbids the act itself—never weaken a control so that code works or a test passes, and a control that can be switched off is already weakened, whether the switch is a flag, an environment variable, or a bypass called temporary—and marks the four highest-risk rules non-negotiable. Compliance also decays over long sessions; [Shukla et al. (2025)](https://arxiv.org/abs/2506.11022) saw critical vulnerabilities increase after only five rounds of a model improving its own code, so the rules end with a self-review of the agent's own diff instead of trusting an instruction read once at the start. The remaining 8.3% is why this is a guardrail, not a control.
