...
The goal is secure code by default: the agent applies these practices on its own, also when the prompt does not mention security at all. Three findings from current research explain how instructions reach that point, and each one is visible in the way this baseline is written.

**Give the agent instructions it can follow, not information it should know.** [Gloaguen et al. (2026)](https://arxiv.org/abs/2602.11988) tested repository context files on real tasks. The project descriptions that most templates start with had no effect, while explicit instructions in the same files were followed. Precision matters as well: [Kharma et al. (2026)](https://arxiv.org/abs/2605.24298) found that asking a model to be security-aware did not reduce the number of vulnerabilities it produced, it only changed which ones appeared. A general request to write secure code is therefore not enough. For this reason every line of the baseline is a requirement that names the mechanism, for example authorize on the server, use parameterized queries, or take the first admin credential from external configuration, instead of a goal that the agent has to interpret.

**Say what must not be given up, because pressure is the moment when it is.** A deadline, a failing test, or a user who asks for a faster result are the typical situations in which security is dropped first. In the AgentPressureBench study mentioned above, agents that were pushed to improve a score took shortcuts in every task, and the stronger the pressure, the earlier this happened. When the rules forbade exactly this behavior, it went down from 100% of the tasks to 8.3%. The baseline therefore describes the shortcut itself and not only the principle: never weaken a control so that code works or a test passes, and a control that can be switched off is already weakened, no matter whether the switch is a flag, an environment variable, or a bypass that is called temporary. The four rules with the highest risk are marked as non-negotiable for the same reason. The remaining 8.3% is the reason why this baseline is a guardrail and not a control.

**Do not rely on rules that are read only once at the beginning.** [Gamage (2026)](https://arxiv.org/abs/2604.20911) found that prohibitions become weaker the longer a session runs, while requirements in positive form stay stable. [Shukla et al. (2025)](https://arxiv.org/abs/2506.11022) observed that critical vulnerabilities increased after only five rounds in which a model was asked to improve its own code. Compliance decreases exactly where agentic work spends most of its time. The baseline answers this with a final step: before the work is finished, the agent reviews its own diff, reports what it found and fixed, and states the affected controls, the test results, and the open gaps. A lasting solution has to come from the model level, through instruction-hierarchy training ([Wallace et al., 2024](https://arxiv.org/abs/2404.13208)), and not from a file like this one.
