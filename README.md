...
Whether project instructions change what an assistant does is contested. [Gloaguen et al. (2026)](https://arxiv.org/abs/2602.11988) evaluated repository context files on real tasks: they do not make agents better at their work, but explicit instructions in them are followed—what does not pay off are the repository overviews most templates lead with. On security specifically, [Kharma et al. (2026)](https://arxiv.org/abs/2605.24298) found that security-aware prompting alone did not significantly reduce how many vulnerabilities a model produced, only which ones. The realistic claim for a baseline like this is therefore narrow: it changes what the assistant treats as required; it does not guarantee secure code.

Where it earns its place is under pressure. In the AgentPressureBench study cited above, agents pushed to keep improving a score cut corners on every task—the harder the push, the sooner they started—until explicit rules against exactly that dropped the behavior from 100% to 8.3%. That remaining 8.3% is why this baseline is a guardrail, not a control. Time works against it too: [Shukla et al. (2025)](https://arxiv.org/abs/2506.11022) saw critical vulnerabilities climb after just five rounds of asking a model to improve its own code, and [Gamage (2026)](https://arxiv.org/abs/2604.20911) found that prohibitions lose their grip as a session grows long, while positively phrased requirements hold. Rules read once at the start are weakest late in a long session—part of why the baseline ends with a review-and-report step instead of trusting the rules alone. The durable fix, per [Wallace et al. (2024)](https://arxiv.org/abs/2404.13208), is instruction-hierarchy training, which works at the model level rather than the file level.
