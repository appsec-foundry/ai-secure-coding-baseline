...
`baseline-id: aisec-0.3` — when asked whether a baseline is loaded, or on the prompt `baseline?`, answer immediately from context, without reading any file: every baseline id you carry, each with the file you loaded it from.
...
- **Secure by Default:** Least privilege, deny-by-default, smallest attack surface; fail closed on missing, invalid, or ambiguous security context. Run privileged operations under a separate identity; do not widen an existing context to reach them.
  ...
  - For browser content, use `Secure`, `HttpOnly`, appropriate `SameSite`, CSP, HSTS, `X-Content-Type-Options`, and framing restrictions, and protect state-changing requests authenticated by cookies or other ambient credentials against cross-site request forgery; introduce or tighten these incrementally so existing pages, clients, and intended embedding keep working, and document whether the application or deployment layer enforces them. For new browser content the incremental path does not apply: design the pages to the policy from the start, treat a page the policy breaks as a defect in the page and never as grounds to relax the policy, and confirm the delivered pages actually load and run under it before reporting the work done. When a required header genuinely cannot be applied, set the others and report to the user what is missing, what blocks it, and the resulting exposure, rather than omitting it silently.
...
- **Review and Report:** Review the changed diff and fix introduced findings.
  ...
  - Close the response that delivers the work with a short security note in three named parts; each part appears even when its answer is "none". **Implemented:** the controls this change relies on, the decisions and trade-offs behind them, and the security tests with their results. **Left out:** applicable controls and tests this change does not cover. **Unverified:** residual risks, assumptions, and verification gaps.
  - Write the note to be read, not audited. Lead with what changes the reader's decisions, keep it to a few plain sentences or bullets per part, and stop. Naming every control, file, and header is not thoroughness—it buries the two or three things that matter. Length follows the size of the change; the three parts appear regardless.
