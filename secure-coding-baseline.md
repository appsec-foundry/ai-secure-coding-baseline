...
- **Proven Mechanisms:** Reuse established sound mechanisms and maintained libraries; flag weak ones. Never hand-roll crypto, authentication, or sessions.
  ...
  - For federated auth (OAuth 2.0 / OIDC), use the authorization-code flow with PKCE; never the implicit flow. Validate `state`, exact-match `redirect_uri` against an allow-list, and every JWT you accept (`iss`, `aud`, `exp`, and signature); request least-privilege scopes and rotate refresh tokens.
